...
namespace MultiplayerARPG.MMO
{
    public class ClusterServer : LiteNetLibServer
    {
        ...
        public ClusterServer(CentralNetworkManager centralNetworkManager) : base(new LiteNetLibTransport("CLUSTER", 16, 16))
        {
	        ...
	        // Tournament
	        RegisterMessageHandler(MMOMessageTypes.TournamentFinish, HandleTournamentFinish);
	        RegisterMessageHandler(MMOMessageTypes.TournamentReset, HandleTournamentReset);
	        // Tournament
#endif
        }
	    ...
	    // Tournament
#if (UNITY_EDITOR || UNITY_SERVER) && UNITY_STANDALONE
	    private void HandleTournamentReset(MessageHandlerData messageHandler)
	    {
		    TournamentResetMessage message = messageHandler.ReadMessage<TournamentResetMessage>();
		    SendPacketToAllConnections(0, DeliveryMethod.ReliableOrdered, MMOMessageTypes.TournamentReset, (writer) =>
		    {
			    writer.PutValue(message);
		    });
...
#if (UNITY_EDITOR || UNITY_SERVER) && UNITY_STANDALONE
	    private void HandleTournamentFinish(MessageHandlerData messageHandler)
	    {
		    TournamentFinishMessage message = messageHandler.ReadMessage<TournamentFinishMessage>();
		    SendPacketToAllConnections(0, DeliveryMethod.ReliableOrdered, MMOMessageTypes.TournamentFinish, (writer) =>
		    {
			    writer.PutValue(message);
		    });
	    }

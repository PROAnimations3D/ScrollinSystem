...
{
    [CreateAssetMenu(fileName = "Reset Dialog", menuName = "Tournament/Reset Dialog", order = -5576)]
    public partial class TournamentResetDialog : BaseNpcDialog
    {
        ...
        public override void GoToNextDialog(BasePlayerCharacterEntity characterEntity, byte menuIndex)
        {
            ...
            switch (menuIndex)
            {
                case CONFIRM_MENU_INDEX:
                    ...
                    {
                        GameInstance.ServerGameMessageHandlers.SendGameMessageByCharacterId(characterEntity.Id, UITextKeys.UI_SUCCES_RESET_TOURNAMENT);
                        BaseGameNetworkManager.Singleton.ResetTournament();
                    }
                    break;
